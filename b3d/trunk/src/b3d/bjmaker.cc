...
int CBjMaker::MakeEvent(){
	...
	if(gaussian){
		...
		if(balance) GenerateParticles_Gaussian_Balance(nparts);
...
int CBjMaker::GenerateParticles(){
	...
	for(ires=0;ires<nres;ires++){
		...
		Ni=rint(floor(density[ires]*volume*nsample));
		...
		for(i=0;i<Ni;i++){
			...
			b3d->part[nparts]->Init(resinfo->code,x,y,tau,eta,p[1],p[2],resinfo->mass,rapidity);
			nparts+=1;
		}
		resinfo=resinfo->nextResInfoptr;
	}
...
void CBjMaker::GenerateParticles_Gaussian(int nparts){
	...
	for(ipart=0;ipart<nparts;ipart++){
		...
		mt=sqrt(m*m+p[1]*p[1]+p[2]*p[2]);
		rapidity=eta+asinh(p[3]/mt);
		b3d->part[ipart]->Init(resinfo->code,x,y,tau,eta,p[1],p[2],m,rapidity);
	}
}
...
void CBjMaker::GenerateParticles_Gaussian_Balance(int nparts){
	...
	for(ipart=0;ipart<nparts/2;ipart++){
		...
		for(ii=0;ii<2;ii++){
			...
			mt=sqrt(m*m+p[1]*p[1]+p[2]*p[2]);
			rapidity=eta+asinh(p[3]/mt);
			b3d->part[2*ipart+ii]->Init(ID,x,y,tau,eta,p[1],p[2],m,rapidity);
		}
	}
