...
exp.density.fn <- function(exp.data.scaled, z.prop){
  nzdim <- length(z.prop)
  density <- rep(0, nzdim)
  ## we sample z.prop on +- pi/2 and then unwrap this to -+ infty using atan(z.prop) 
  z.prop <- atan(z.prop)
  for(i in 1:nzdim){
...
scale.obs <- function(fn.data, exp.data){
  ...
  result$obsError <- (result$obsError)/(scale.vec.obs**2)
...
samples.final <- generate.mc.samples(fnData, expData.scaled, exp.density.fn, nstep.mc=500000)
