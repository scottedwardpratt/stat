...
def main():
    ...
    Emulator = subprocess.Popen(['test_Emu++',Statefile], stdin=subprocess.PIPE, shell=False, stdout=subprocess.PIPE)
    ...
    for i in Xpoints:
        ...
        time.sleep(1)
        points.append(map(float,Emulator.stdout.readline().split(" ")[2:-1]))
        means.append(map(float,Emulator.stdout.readline().split(" ")[2:-1]))
        errors.append(map(float,Emulator.stdout.readline().split(" ")[2:-1]))
        diff = 0
        index2 = 0
        temp = []
