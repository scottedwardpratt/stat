...
## converts cclass values into a [lower,upper] array
def classIndexToRange(index)
  indexMax = 5 # change this if there are any higher classes available
  if(index < 0) 
    puts "cclass index should be positive"
    exit -1
  end
  
  if(index > indexMax)
    puts "cclass index: #{index} is out of range (max: #{indexMax})"
    exit -1
  end
  rangeLower = (index -1 ) * 10
  rangeUpper = index * 10
...
def outputHeader(outputfile, centrange)
  outputfile.print("# combined data from runs in #{@processPath}\n")
  outputfile.print("# centrality from #{centrange[0]} to #{centrange[1]}\n#\n")
  outputfile.print("# generated by: generate-analysis-files.rb on #{Time.now}\n")
  outputfile.print("#\n")
end

## print some header info to the outputfile
def outputDefaultHeader(outputfile, centrange)
...
@centClassIndex = ARGV[1].to_i
@centRange = classIndexToRange(@centClassIndex)
@centFolderString = "cent#{@centRange[0]}to#{@centRange[1]}"
if(ARGV.length > 2)
...
puts "# in cent range: #{@centRange[0]} to #{@centRange[1]}"
...
outputHeader(outputFile, @centRange)
...
outputDefaultHeader(defaultFile, @centRange)
