# lets run the emulator over the met data
...
modelOutputFile <- "./wave-1/metallicity_MV_outputs.dat"
...
expDataFile <- "./metallicity_MV_observed.dat"
expDataIn <- as.matrix(read.table(expDataFile))
expData <- list( obsValue=expDataIn[1,], obsError=expDataIn[2,])

# do we want to rebuild the hyperparameters or not? 1 for yes
rebuild <- 0
buffer <- paste("./thetas-comb-met.dat")

if(rebuild == 1){
  estimResult <- doCombEstimation()
...
rptForNotes <- function(){
  ...
  write.table(round(thetasFin,3), "thetas-table-met.dat", row.names=FALSE, col.names=FALSE)
  ...
  # hold back ratios
  rats <- holdBackRatios()
  write.table(round(rats,3), "hold-back-ratios-met.dat", row.names=FALSE, col.names=FALSE)
...
emuStepPlots <- function(){
  for(i in 1:nbins){
    buffer <- paste("./images/emu-mean-stepped-met-bin-",i,".pdf", sep="")
    cat("emuStepping: ", buffer, "\n")
    pdf(buffer)
    stepPlotDimension(estimResult,i,1,2,3)
    dev.off()
  }
}
...
implausSweep <- function(){
  for(i in 1:nbins){
    buffer <- paste("grid-implaus-met-bin-", i, ".csv", sep="")
    cat("sweeping: ", buffer, "\n")
...
impStepPlots <- function(){
  for(i in 1:nbins){
    buffer <- paste("./images/implaus-stepped-met-bin-",i,".pdf", sep="")
    cat("implausStepping: ", buffer, "\n")
    pdf(buffer)
    stepPlotDimensionImplaus(i,1,2,3, estim.result=estimResult, exp.data=expData)
    dev.off()
  }
}
...
impStepPlotsComb <- function(){
    buffer <- paste("./images/implaus-stepped-comb.pdf", sep="")
    cat("implausStepping: ", buffer, "\n")
    pdf(buffer)
    stepPlotDimensionImplausComb(1,2,3, estim.result=estimResult, exp.data=expData)
    dev.off()
}
...
doEstTestLhoodPlot <- function(){
  for(i in 1:nbins){
    buffer <- paste("./images/lhood-compare-bin-", i, ".pdf", sep="")
    pdf(buffer)
