...
source("fnAnalysis.R") # load the functional data helpers
...
generate.mc.samples <- function(fn.data, exp.data, data.density.fn, nprocs=4, nstep.mc=500000){
  ...
  xmin <- apply(fn.data$model.sample$des, 1, min)
  xmax <- apply(fn.data$model.sample$des, 1, max)
  
  ic.x <- matrix((xmax-xmin)*runif(nparams*nprocs, 0, 1)+xmin, nrow=nprocs, ncol=nparams)
  ic.z <- matrix(runif(nbins*nprocs, -pi/2, pi/2), nrow=nprocs, ncol=nbins)

  ##xr.in <- matrix(c(0,1), nrow=nparams, ncol=2, byrow=TRUE)
  xr.in <- cbind(xmin, xmax)
