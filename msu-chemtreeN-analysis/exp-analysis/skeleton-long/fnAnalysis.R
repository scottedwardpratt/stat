...
fn.sample.gen <- function(cov.fn.in=1, reg.order.in=1 ){
  ...
  retVal <- list(model.sample=sample, exp.obs=expData, cov.fn=cov.fn.in, reg.order=reg.order.in, nruns=dim(modelData)[1], nbins=nbins, nparams=dim(designData)[2])
...
fn.emulate.slice <- function(fn.data, dimA, dimB, fixedValVec=NULL, nemupts=32){
  ...
  for(i in 1:nemupts){
    for(j in 1:nemupts){
      ...
      ## copy in the rest of the non na entreis from
      ## fixedValVec 
...
fn.implaus.slice <- function(fn.data, slice.data){
  ...
  for(i in 1:npts){
    for(j in 1:npts){
      ...
      V.mat.inv <- solve(V.mat)
      ## we should include the systematic errors carefully here also, the errors should 
      ## probbaly be somehow proportional to the mean of the data?
...
fn.plot.slice <- function(fn.data, slice.data, plot.thing, unscale=TRUE, legend.in="",
  ...
  if(imp==FALSE){
    ...
    breaks <- c(0, ils)
    nlevels <- length(ils)
    image(range1, range2, plot.data, axes=FALSE, col=rev(cm.colors(nlevels)), breaks=breaks,
          xlab=xlabel, ylab=ylabel)
    contour(range1, range2, plot.data, levels=ils, labels=ils, col="black", add=TRUE, cex.lab=0.5, labcex=0.8)
  }
    
  cat("# legend.in: ", legend.in, "\n")
  #legend("topright", legend.in, bg="white")
  if(plot.des==TRUE){
...
fn.emu.steps <- function(fn.data, dimA, dimB, stepDim, fixedValVec=NULL, nsteps=9, range.Min=NULL, range.Max=NULL){
  ...
  for(i in 1:nsteps){
    ...
    sliceList[[i]] <- fn.emulate.slice(fn.data, dimA, dimB, fixedValVec=fixedStep)
    stepVec[i] <- stepVal
  }
...
fn.plot.steps <- function(fn.data, step.list, obsIndex, plot.var=FALSE){
  ...
  if(is.null(step.list$fixedVals)==FALSE){
    if(plot.var==FALSE){
      buffer <- paste("E(obs: ", obsIndex, ") fixed: ", fixedVals)
    } else {
      buffer <- paste("Var(obs: ", obsIndex, ")fixed: ", fixedVals)
    }
  } else {
...
fn.plot.imp.steps <- function(fn.data, step.list, obsIndex, plot.joint=TRUE, title=NULL){
  par(mfrow=c(3,3), mar=c(2,2,1,1), oma=c(4,5,3,0))
  ...
  if(plot.joint == FALSE){
    if(is.null(step.list$fixedVals)==FALSE){
      ...
      if(is.null(title)){
        ...
        buffer <- paste(title, " obs: ", obsIndex)
      }
    }
  } else {
    buffer <- paste("joint implaus")
  }
