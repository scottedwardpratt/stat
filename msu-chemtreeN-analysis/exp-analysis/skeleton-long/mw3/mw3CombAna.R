...
## to the script to analyse the data comparing against the mw3 observables
...
##
## lum
expDataFileLum <- "./lum_fun_observed_MW3.dat"
expDataLum <- as.matrix(read.table(expDataFileLum))

expDataFileMet <- "./metallicity_MV_observed_MW3.dat"
...
fn.plot.imp.steps(fnData, impSteps, plot.joint=TRUE, title.in="MW3 ")
dev.off()

# lets plot all the individual implausibilities in this projection
## for(i in 1:nbins){
##   pdf(paste("./images/implaus-obs-",i,"param-1-3-2.pdf", sep=""))
##   fn.plot.imp.steps(fnData, impSteps, i, plot.joint=FALSE, title.in="MW3 ")
##   dev.off()
## }
