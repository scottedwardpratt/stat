...
dump.grid.inde.csv <- function(data.grid, outstub){
  npts <- dim(data.grid$grid)[1]
  ...
  nbins <- length(data.grid$inde)
  for(index in 1:nbins){
    final.dat <- cbind(data.list.output, as.vector(data.grid$inde[[index]]))
    colnames(final.dat) <- c("x", "y", "z", "implaus")
    buffer <- paste(outstub, "-obs-", index, ".csv",sep="")
    write.csv(final.dat, file=buffer, row.names=FALSE)
  }
}
