...
double Distribution::IntegratedNormal(double x, double mu, double sigma, double data_sigma){
	double likelihood=0;
...
double Distribution::Gaussian(gsl_vector x, gsl_vector mu, gsl_matrix sigma, gsl_matrix data_sigma){
	double out=1;
	int n=x.size;
	for(int i=0;i<n;i++){
