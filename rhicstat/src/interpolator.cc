#ifndef __INTERPOLATOR_CC__
#define __INTERPOLATOR_CC__
#include "coral.h"
#include "rhicstat.h"
using namespace std;

CZGetter_Interpolated::CZGetter_Interpolated(CRHICStat *rsptr) : CZGetter(){
	LINEAROFF=true;
	rhicstat=rsptr;
	hyperR_default=parameter::getD(rhicstat->parmap,"HYPERR_DEFAULT",0.1);
	READ_HYPERPARS=parameter::getB(rhicstat->parmap,"READ_HYPERPARS",false);
	LINEAROFF=parameter::getB(rhicstat->parmap,"INTERPOLATOR_LINEAROFF",false);
	NORMALIZE=parameter::getB(rhicstat->parmap,"INTERPOLATOR_NORMALIZE",true);
...
void CZGetter_Interpolated::LinearFit(){
	int ix,jx,iz,irun;
...
void CZGetter_Interpolated::InitInterpolator(){
...
double CZGetter_Interpolated::GetCov(int iz,CRunInfo *runinfo1,CRunInfo *runinfo2){
	return GetCov(iz,runinfo1->x,runinfo2->x);
}

double CZGetter_Interpolated::GetCov(int iz,double *x1,double *x2){
	...
	if(sum>200) sum=200;
	answer=hyperTheta0[iz]*exp(-0.5*sum)+hyperNugget[iz];
	//answer=1.0-sum/(hyperR_default);
	return answer;
}

void CZGetter_Interpolated::GetCovDerivs(int iz,double *x1,double *x2,double &C,double *DC,double **DDC){
...
void CZGetter_Interpolated::GetZ(double *x,double *z){
	int iz,izz,irun,ix;
	double norm,cov;
	for(iz=0;iz<NZ;iz++){
		z[iz]=blinear[iz];
...
void CZGetter_Interpolated::ReadHyperPars(){
...
void CZGetter_Interpolated::PrintHyperPars(){
const double root3=sqrt(3.0);
	char dummy[150];
	int iz,ix,id;
	double scale,nugget;
...
void CZGetter_Interpolated::CalcHyperPars(){
	//Use Newton's method
