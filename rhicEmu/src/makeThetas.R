...
cargs <- Sys.getenv(c('infile', 'errfile', 'paramfile'))
inSamples <- cargs[1]
...
save(sample, file="SampleTemp.dat")
...
save(pca.decomp, file="PcaTemp.dat")

# finally estimate the thetas
runTime <- system.time(thetas.est <- estimateThetas(pca.decomp))
write.table(thetas.est, "theta-table.dat", row.names=FALSE, col.names=FALSE) # and dump to file
